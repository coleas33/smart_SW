...
generator writes. The generator reclassifies the recorded runs' touching groups - 3, 2 and 0 -
as the contacts feature 010 records them as, by the replay's own rule, so the fixtures record
those contacts and a replay of them reclassifies none; every pass with the recorded settings
records the same contacts again, and the requested passes hold them among theirs.
...
@pytest.mark.parametrize("name", NAMES)
def test_the_pane_defaults_record_every_recorded_contact_among_theirs(
    name: str, tmp_path: Path
) -> None:
    """Checks first judges every group, so the requested pass records more contacts than the
    recording (every fictional row of the big fixture touches); never fewer."""
    passes = replay_passes(
        read_recording(FIXTURES / name),
        tmp_path,
