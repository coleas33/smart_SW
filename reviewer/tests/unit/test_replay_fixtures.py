...
to the prompt or the checklist, regenerates them, and the 1% bar holds against what the
generator writes. The recorded runs' touching groups - 3, 2 and 0 - now live in three places:
the generator's reclassification output (it reclassifies them, by the replay's own rule, as the
contacts feature 010 records them as), the fixtures' recorded contacts, and the replay
reproducing those contacts exactly with the recorded settings. So a replay of a fixture
reclassifies 0, 0 and 0, under every requested setting, and every requested pass holds the
recorded contacts among its own. The replay's reclassification path stays pinned on the
scripted pre-010 recordings of `test_replay_findings.py`. That no recorded finding is lost or
not replayable is unchanged, and absolute.
...
RECORDED_CONTACTS = {"big-assembly": 3, "small-assembly-a": 2, "small-assembly-b": 0}
"""The touching groups (0.0 mm3) each fixture records as contacts. The recorded runs wrote them
as `interference.static` findings; the generator reclassifies them by the replay's rule and
prints the count. Until the fixtures were regenerated by the current code (decision 3A,
2026-09-23) they held the findings, and the replay reclassified 3, 2 and 0 (010 T094-T095);
now it reclassifies 0, 0 and 0 (`no_recorded_finding_lost`) and these are pinned instead."""
...
def test_the_finding_set_is_exact(name: str) -> None:
    ...
    no_recorded_finding_lost(findings)
    assert findings.added == []
    assert findings.recorded == findings.replayed


@pytest.mark.parametrize("name", NAMES)
def test_each_fixture_records_its_touching_groups_as_contacts_not_findings(name: str) -> None:
    session = read_recording(FIXTURES / name).session
    recorded = contact_groups(session)

    assert sum(recorded.values()) == RECORDED_CONTACTS[name]
    assert {contact.kind for contact in session.contacts} <= {"zero_volume"}
    judged = {judged_group(finding) for finding in session.findings} - {None}
    assert not judged & set(recorded)


@pytest.mark.parametrize("name", NAMES)
def test_every_recorded_contact_is_replayed_with_the_recorded_settings(
    ...
    assert contact_groups(passes.second.session) == contact_groups(passes.recording.session)
...
def test_the_big_assembly_has_one_estimated_round_the_live_call() -> None:
    ...
    were findings and the current code's contacts, which a replay could not tell apart from
    anything the live call did. Now the fixture records the contacts, so they reproduce."""
    report = as_recorded("big-assembly")

    estimated = [r for r in report.rounds if r.estimated]
    assert len(estimated) == 1
    [call] = [c for r in estimated for c in r.calls if c.class_ == "estimated"]
    assert call.tool == "bridge_interference"
    groups = [c for r in report.rounds for c in r.calls if c.tool == "check_interference_group"]
    assert len(groups) == 6
    assert {c.class_ for c in groups} == {"reproduced"}
...
def test_every_requested_setting_loses_no_recorded_finding_and_holds_every_recorded_contact(
    ...
    passes = replay_passes(
        read_recording(FIXTURES / name),
        tmp_path,
        requested=REQUESTED_SETTINGS[setting](name),
        standards_profile=EXAMPLE_PROFILE,
    )
    ...
    assert recorded - requested == Counter()
    assert sum(recorded.values()) == RECORDED_CONTACTS[name]
