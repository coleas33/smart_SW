...
the fictional root and no recorded design id appears.
"""
...
GENERATOR = BIG_ASSEMBLY.parents[1] / "pane" / "generate_pane_fixture.py"
...
def test_every_path_sits_under_the_fictional_root(fixture: dict[str, Any]) -> None:
    ...
    outside = [
        text
        for text in strings_of(fixture)
        if DRIVE_PATH.search(text) and not text.startswith(FICTIONAL_ROOT)
    ]
    assert outside == []


def test_no_recorded_design_id_appears(committed: str) -> None:
    assert [one for one in RECORDED_DESIGN_IDS if one in committed] == []
