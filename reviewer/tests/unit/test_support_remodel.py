...
def test_every_package_is_at_the_schema_version_the_planner_receives(name: str) -> None:
    ...
    fields beside it and 1.4.0 the standards evidence; a 1.1.0 package has none of them, and
    both serializers forbid unknown members, so it can never be one."""
    package = remodel_package(TREE_BUILDERS[name]())

    assert package.schema_version == SCHEMA_VERSION == "1.4.0"
