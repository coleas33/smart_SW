...
| `pymupdf` | AGPL-3.0-or-later, or a commercial license from Artifex | Drawing-PDF text extraction. This project is itself AGPL-3.0-or-later (see `LICENSE`), so the dependency is license-compatible; a closed build would need Artifex's commercial license or a replacement reader. |
