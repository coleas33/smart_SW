...
| `SwReview.AddIn.Tests/`: the Review refusal sentence | *landed as* also `ReviewHostTests`' run-directory section, `ToolServiceWiringTests`' `drawing.read` section (T073) with `FakeAppThread.cs` NEW (moved out of `InProcPipeServerTests.cs`), and `ReviewPageDrawingQuestionsTests` over the generated `Fixtures/review-drawing-questions.json` (`reviewer/tests/fixtures/pane/generate_drawing_questions.py`, `test_pane_drawing_fixture.py`) |
