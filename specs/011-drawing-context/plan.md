...
| `SwReview.AddIn/ToolService/ToolServiceHost.cs` CHANGED (Q2) | *landed as* written (T074, 2026-09-23): `ToolServiceOptions.ReviewRunDirectory`, `ConfirmedDrawingSource` set on `BridgeServices.ConfirmedDrawings` by `Attach`, and the request chain extracted as `RequestChain`; `SwReviewAddIn.cs` CHANGED (the option set from the review host) |
| `SwReview.AddIn/Review/ReviewHost.cs`: the sentence | *landed as* also `ReviewRunDirectory(run_id)` (T074's half) |
| `SwReview.AddIn.Tests/`: the Review refusal sentence | *landed as* also `ReviewHostTests`' run-directory section, `ToolServiceWiringTests`' `drawing.read` section (T073, and T089's end-to-end test) with `FakeAppThread.cs` NEW (moved out of `InProcPipeServerTests.cs`), and `ReviewPageDrawingQuestionsTests` over the generated `Fixtures/review-drawing-questions.json`, read through `DrawingQuestionsFixture.cs` NEW (T091), which `SummarySample`'s drawing line shares (`reviewer/tests/fixtures/pane/generate_drawing_questions.py`, `test_pane_drawing_fixture.py`) |
