...
| POST | `/sessions` | `{run_dir, provider, model, effort, bridge: {pipe, secret} \| null, engineer, retry_of: chat_id \| null, standards_profile: str \| null}` | `201 {chat_id, review_session_id, not_examined}`; starts the review turn immediately. `standards_profile` is the configured profile path, passed to the reasoning side to attach `check_standards`; blank or null means no profile. An unreadable or invalid configured profile leaves the review possible and makes the standards family not evaluated, rather than refusing the review. `not_examined` is the shared `{sentence, instances}` block from `report/unexamined.py`, or null when every instance was read; Review displays it above Start here as soon as the session starts. 400 if `run_dir` fails the path rule below or `run_dir/package.json` is missing or invalid. |
