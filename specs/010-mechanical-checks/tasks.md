...
- [x] T075 [US7] Implement `check_hygiene` in `reviewer/src/swreview/tools/checks_mechanical.py`, register it, append it to `CODE_FIRST_CHECKS`, classify the five ids as `hygiene`, regenerate the payload constants and the `levers.md` byte counts. **Shared with 008** (as T067). Acceptance: T074 green; FR-019
...
- [x] T092 [008] Extend `reviewer/tests/unit/test_prerun_repeat_guard.py` (008 T043): `repeat_key` maps `check_joints`, `check_mass_material` and `check_hygiene` to `(tool,)`; a repeat after the pre-run records one step, no finding, and returns the recorded digest. After 008 T044
- [x] T093 [008] Add the three keys to `repeat_key` in `reviewer/src/swreview/prerun.py` and document them in `specs/008-checks-first-review/contracts/checks-first.md` section 5 and `specs/001-agentic-design-review/contracts/agent-tools.md`. Acceptance: T092 green
...
- [x] T098 Extend `reviewer/tests/unit/test_checklist.py`: two items `mass.material` (`check_prefix: mass.`) and `hygiene` (`check_prefix: hygiene.`) load and close on their findings and on their summary coverage; and **edit deliberately** `tests/integration/test_coverage_stop.py`'s round counts and spare call indices, the `cover-blind-tap` golden (regenerated for a diff of exactly two more open checklist items) and `report/attention.py CHECKLIST_ITEM_IDS`' guard in `test_attention.py`. After T067 and T075
- [x] T099 Add the two items to `reviewer/src/swreview/agent/checklist_v1.yaml` and to `CHECKLIST_ITEM_IDS` in `reviewer/src/swreview/report/attention.py` (**shared with 008**). Acceptance: T098 green; `test_prefix_stability.py` green. *Landed as* (2026-09-23, T098 with it, after T108): the patch the first attempt wrote on an older main, adapted - the two descriptions name no tool and end "Closed by a mass (hygiene) finding, or by the family's summary coverage item", as `standards.release` does (research R2.25), so lever 13's table and `test_withheld_wording.py` stay unedited; `test_checklist.py` adds that neither item names its tool and that each item's id is its family's `SUMMARY_CHECK`; `test_coverage_stop.py` derives its round counts from the checklist (`ITEMS`) rather than retyping twelve; `review_words_v1.yaml`'s mass-and-material and hygiene goals list the new items. The `cover-blind-tap` golden was regenerated with `--force-regen` for exactly two more open items. The change moved feature 008's three replay fixtures past SC-001's 1% (1.55%, 1.75%, 1.75%), so they were regenerated in the same commit (008 T121, owner decision 3A), and with them the pane fixture and its pins; `test_ir_golden_fixtures_load.py`'s golden-tree check reads `git status` and passes once the golden is committed
- [x] T108 *(amendment 2026-09-23, research R2.25)* Before T099: extend `reviewer/tests/unit/test_tools_check_mass_material.py` and `reviewer/tests/unit/test_tools_check_hygiene.py` (red first): each tool writes exactly one coverage row under its checklist item's id (`checks/mass.SUMMARY_CHECK` = `mass.material`, `checks/hygiene.SUMMARY_CHECK` = `hygiene`, each equal to a checklist item's id), `checked`, its reason counting the rows the call wrote, the documents it walked and the findings it recorded, its scope the reviewed configuration; a repeated call leaves one; the result's counts and keys are unchanged; the hygiene row is written with and without a standards profile; and a run with no finding closes the item (`load_checklist().buckets(session)`), where before it stayed open. Then write the rows in `reviewer/src/swreview/tools/checks_mechanical.py`. The two items' descriptions name no tool (lever 13's sweep, `test_withheld_wording.py`, stays green unedited). Acceptance: the new tests green; `test_withheld_wording.py` green. *Landed as* (2026-09-23): the tests are one module parametrized over the two families, `reviewer/tests/unit/test_mechanical_family_summaries.py`, rather than a copy in each tool's file; the row is written by one helper, `checks_mechanical._record_summary`. On `prerun_package` a hygiene run records no finding, and its item is closed by the row alone. `test_tools_check_hygiene.py`'s set of checked rows gains `hygiene`, deliberately; the results, the digest, the replay fixtures and the pane fixture do not move. *Amended* (2026-09-23, a defect found in review, research R2.25's amendment): the row was always `checked`, so a run that checked nothing read "checked" on the Review summary's goal line; it is now `checked` only on a `checked` per-check row or a recorded finding, `skipped` otherwise with the same counts, `failed` on a refused finding, one row across the three (`checks_mechanical.SUMMARY_BUCKETS`). Tests, red first in the same module: all skipped, one checked, findings only, a refusal (first, with passes, after a finding), each with its goal line; a repeat that moves the row; every part lightweight through both families; `prerun_package`'s hygiene run now closes its item `skipped`. The two tools share one body, `checks_mechanical._record_family`. *Amended again* (2026-09-23, a defect found in review, research R2.25's second amendment): after a refused call the saved session carried finalization's close-out row beside the `failed` one, whose false sentence took the goal line to "evidence missing"; `runner.finalize_session` now adds no close-out row for an item with a `failed` row under its own id. Tests, red first in the same module: every scripted case keeps its row and its goal line through finalization; a `failed` row under `tool.<name>` leaves the close-out
- [x] T100 [P] Update `README.md` (the three checks in the review section; the owner's engagement rule; profile version 2 for the owner's real profile) and `specs/007-attention-policy-gate/contracts/attention.md` (the twelve classes). *Landed as* (2026-09-23): a paragraph under Run in `README.md`; in `attention.md`, a table of the twelve ids with their classes and the tool that emits each, at the end of section 1
- [x] T101 [P] Run `quickstart.md` Scenarios 0 to 11 and the regression gate (`uv run pytest -q`, `uv run ruff check src tests`, `dotnet build`/`dotnet test SwReview.sln -c Release`), including `reviewer/tests/perf/test_joint_map_perf.py` (`-m perf`: `build_joint_map` under 200 ms, `check_joints` under 2 s on the big fixture); fix anything they surface in the task that owns it. *Landed as* (2026-09-23): `tests/perf/test_joint_map_perf.py` did not exist - `plan.md` lists it as new and no earlier task wrote it - so it is written here: `build_joint_map` on the big fixture 0.097 s against 0.2 s, `check_joints` through the dispatch, mesh loads included, 0.449 s against 2 s, each asserting the contract's workload (132 instances, 50 joints). Scenarios 0 to 12 green offline, every C# filter they name included; the regression gate green (8,859 passed, 10 skipped; ruff clean; `dotnet build` 0 warnings; `dotnet test` 2,317 and 1,485 passed). Corrected in `quickstart.md`: `$big` and `$small` name the fixture folders and Scenario 2's one-liner reads `.package` (it raised `FileNotFoundError`); Scenario 12 reclassifies three recorded findings, not five; the perf test is in the regression gate; and (found in T102) Scenario 6's `FHT` heads take the Torx key since the owner's answer - the unresolved path is a head with no stated drive and no head tool
- [x] T102 [P] Re-validate `specs/010-mechanical-checks/checklists/requirements.md` against the amended spec (research R4); reconcile `plan.md`'s Source Code block with what landed. *Landed as* (2026-09-23): the checklist holds against research R4 and R5's answers, with the real profile at version 2, the twelve classes' confirmation and T098-T099 recorded as open. `plan.md`'s Source Code block marks `agent/runner.py` as changed once (`7b63519`) where it said reused unchanged, adds the files it did not name (`checks/documents.py`, `tools/joint_context.py`, `tools/measure.py`, the real Hole Wizard and tolerance readers, the gate and circuit-breaker change, the IR 1.5.0 edits beyond its rows, the new tests and goldens, 008's two contracts, the docs), and marks what did not land (T098-T099's checklist items and tests; `Ir/PackageSerializer.cs` and `IrContract.cs` needed no change; `test_schema_sync.py` passes unedited)
- [x] T109 [P] *Added 2026-09-23 (seat-readiness review: T103's "record which read path answered" had nothing to read)*: the command-line dump watches its gate and ends `extract.log` with the member set. `extractor/SwReview.Extractor.Tests/CommandLineOptionsTests.cs`: `Program.DumpGate(observer)` carries the observer and the read-only guard (a read passes, `SetSuppression2` and `Save3` are refused, every name in the set, both refusals recorded); `Program.DumpGateLogLine` writes `gated=` and each member once in first-seen order. `extractor/SwReview.Extractor.Console/Program.cs`: `ExecuteDump` attaches through `DumpGate` and writes the line after the counts, and after a failure too. `PropertyDumperTests` already pins that `GetOverrideOptions` is asked only when the first path gave no answer. Acceptance: red for want of the two members, then green; zero warnings; the dump's package unchanged *Landed as* (2026-09-23): as written; the pane's own dump (`SwReviewDump`, in the add-in) is not this command and is unchanged, so the seat dumps with `swreview-extract` for these tasks

---

## Phase 13: The next workstation sitting

**The dumps T103 to T106 read** (*added 2026-09-23 on review*): with the assembly the engineer opened active in SOLIDWORKS, and `swreview-extract` aliased as runbook section 1 says, `swreview-extract dump --out <a new folder under the run root> --meshes none` - the active document and configuration, the full profile, the needed faces, the feature tree and the equations are the defaults, and meshes are not what these tasks read. It writes `package.json` and `extract.log` into that folder and nothing else. Then, from `<repo>\reviewer`, one command prints every count the four tasks ask for:

```powershell
uv run python -c "import sys; from collections import Counter; from swreview.ir.loader import load_package; from swreview.checks.fastener_identity import recognise_fasteners; p = load_package(sys.argv[1]).package; d = p.documents; print('documents', len(d), '| mass_overridden read', sum(x.mass_overridden is not None for x in d), '| mass_override gaps', sum(g.entity_kind == 'mass_override' for g in p.gaps)); w = [h for h in p.holes if h.wizard is not None]; print('holes', len(p.holes), '| Hole.wizard filled', len(w), '| fit classes', sorted({h.wizard.fit_class_raw for h in w if h.wizard.fit_class_raw}), '| thread classes', sorted({h.wizard.thread_class_raw for h in w if h.wizard.thread_class_raw})); print('model_dimensions', len(p.model_dimensions), '| model_annotations', len(p.model_annotations)); r = recognise_fasteners(p); print('fasteners', len(p.fasteners), dict(Counter(f.identity_source for f in p.fasteners)), '| with a shank face', sum(x.shank_face_id is not None for x in r))" <the dump folder>
```
...
- **Workstation (Phase 13)**: T103 after T069 and T109; T104 after T089; T105 after T091; T106 after T053; T107 after US1 to US7 and 008
...
- T093 after T092; T095 after T094; T099 after T098 and T108
