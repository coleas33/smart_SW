...
- [x] T022 [W] [US2] **The read-through.** The folders are already on the development machine at `%LOCALAPPDATA%\SwReview\handover\runs` (fourteen: the three 2026-09-18 reviews and two checks of the small assembly, a single part's review and check, another assembly's three reviews, and the empty or aborted rest), and the decisions were taken on 2026-09-19 over a preview of the same rule (research R2.15). What remains here is the confirmation: run `swreview attention` over each folder with the owner and check the tool reproduces the agreed orders; any disagreement is an edit to `attention_policy_v1.yaml`'s `classes` or `needs_judgement` and a change to the fixture's expected order in T017. Acceptance: the tool's top five on the small assembly's `20260918-215755-…` is the two interference rows, mates to reference geometry, the under-defined sketch and grouping; on the two check folders the sketch leads; on the other assembly's three reviews the two rebuild-breaker rows lead, the unfixed first component before mates to reference geometry because they tie on every key but the check id (RK-1). Confirmed 2026-09-19 over the ten folders that hold a `session.json`, every folder byte-identical afterwards (research R2.15, second table)
...
- [ ] T059 [W] `quickstart.md` Scenario 11: on the pilot workstation, one review of the small assembly ends with a pinned panel whose rows equal `report.md`'s "Start here"; Model check and Standards show the rows above the chips (SC-011)
...
- [ ] T062 [W] [K] `quickstart.md` Scenario 13: six alternated runs with `--study procedural_gate`, scored, compared; the ledger row's `fell_in_runs` is empty and the median column carries `n=`; the row is committed to `docs/llm-efficiency-options.md` (FR-034, SC-010). Note 2026-09-19: on the one-package set every row carries the too-small override, `compare --check` then exits 1 on the committed document, and `test_check_exits_zero_on_the_committed_document` cannot pass, so the row cannot be committed until the set holds a held-out package; `docs/workstation-handover-2026-09-19.md` Task D keeps the study outside `benchmarks/studies/` and hands `ledger.md` back instead
