...
| VI. Findings inspectable, coverage tracked | Every finding reproducible; coverage on the report; time measured per design | "Start here" adds a reason line per row naming the key that placed it; the coverage block states the bucket counts and the families most left unreached; timing becomes recordable on every real run and the median is reported on the ledger, which is the constitution's own measurement rule made possible for the first time. | PASS |
...
3. **Amplify, never filter, and never write.** Nothing leaves `session.findings`; the fold is a
   ...
   is sourced from the session's buckets and the first dotted segment of each item's check id.
