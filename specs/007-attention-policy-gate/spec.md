...
- **FR-013**: The report MUST carry a "Start here" section of at most five rows immediately above the Findings section, each row with a one-line reason naming the key that placed it, followed by a not-amplified line (counting checked within scope, already decided, informational, and beyond the top five) and a coverage block stating the bucket counts, the checklist items the run closed out as unresolved with the reasons it recorded, the open evidence requests, and the rule counts; no line of the section MUST contain a percent sign.
