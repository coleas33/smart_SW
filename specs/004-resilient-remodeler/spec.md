...
- The add-in reconnects to SOLIDWORKS between Remodel a copy and Start, which it does when the active document changes (the tool service re-attaches, and the bridge session that holds the copy does not survive it): Start is refused by name, before anything is changed and with nothing written to the part, the copy or the run folder, and the message sends the engineer back to plan again (owner decision 22A, 2026-09-25). A reconnect back to the same document is still a reconnect and is refused; a configuration switch or the same document spelt differently reconnects nothing and is not. *Amended 2026-09-25 (owner, decision 24A):* the pane says so as soon as the plan is lost, before Start is pressed - a plain-words line, Start disabled for that plan and Plan again offered - and the refusal at Start stays as the backstop. Not yet in effect: 004 T170 implements it, and until then the pane learns of the loss only at Start.
...
- **FR-031**: The run MUST NOT auto-resume after a lost SOLIDWORKS session. The copy and the change log MUST survive on disk, the toggles MUST be restored, and a resume request MUST be refused. *Amended 2026-09-25 (owner, decision 22A):* the same holds between planning and Start: a plan whose bridge session did not survive a tool-service re-attach MUST be refused at Start, before any change, with a named error in plain words that sends the engineer back to plan again; the session is not carried across the re-attach. *Amended 2026-09-25 (owner, decision 24A):* the pane MUST say so as soon as the plan is lost, before Start is pressed, and MUST NOT offer Start for that plan; the refusal at Start stays as the backstop. Not yet in effect: 004 T170 implements it.
...
- **FR-050a** (*added 2026-09-25, the owner's decision 21A*): The read-only denylist MUST refuse the whole feature-creation family of `IFeatureManager`, `IModelDoc2`, `IPartDoc` and `IModelDocExtension` - every public method a stated creation grammar matches, plus the named creators the grammar does not reach - bare or interface-qualified, with a table **generated from the installed interop's metadata**, never typed, and a reflection test proving it complete. A read the grammar reaches, and the bare name of a stage-1 allowlist key, MUST stay allowed, each named with its reason; no product source literal may name a newly denied member, except the throwaway-part probe's own creation calls, which only the probe's guard MAY exempt and no other gate may.
...
**Back burner (owner decision 2026-09-19):** modelling a STEP or other dumb-solid import with the method. A native part carries a tree to repair; an import carries none, so that path needs feature recognition first (SOLIDWORKS FeatureWorks, which ships with SOLIDWORKS Standard, Professional and Premium alike, or a recogniser of our own) before stage 1 has anything to restructure; whether the FeatureWorks add-in is enabled on the seat has not been checked, and that, not the seat's tier, is the open question (corrected 2026-09-25, decision 17A). The re-modeler continues on SOLIDWORKS files only: the Phase 2 probes and the Phase 9 bring-up of this feature, then stage 2 and assemblies re-specified with those numbers. The import path is reopened only by the owner.
