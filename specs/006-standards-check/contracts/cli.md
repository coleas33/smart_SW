...
| `probe standards` | `--doc <document>` | Prints all ten workstation probes in one read-only run under the read-only guard (`research.md` R4): the exploded read on the document and on each sub-assembly's loaded document, plus the explode-step count for every configuration - read **without activating one**, because activating a configuration rebuilds the model and changes the display state, which this run forbids, so the exploded pair itself is only available for the configuration already active; `HasMaterialPropertyValues` and all nine appearance slots for each component; `Visible` and `GetVisibility` for each component; per sheet, whether the revision table is reachable, whether the cast to the table interface succeeds, `CurrentRevision`, the row and column counts and every cell; per sheet and per view, the text of every note with the sheet-format pseudo-view (`Type == 1`) marked and the `GetViews()` enumeration compared against the `GetFirstView()`/`GetNextView()` walk; per view, the view type, the referenced model name, whether the referenced document handle is non-null, every annotation's name, type and dangling flag, and every display dimension's override flag, override value, type and computed value; the same walk over **every** sheet while sheet 1 stays active, with per-sheet counts so a non-active sheet that came back empty is visible; every feature's type name and name at every depth with body-folder body counts and exclusion flags; `GetSketchTextSegments` for a plain sketch, a text sketch, an empty sketch and a hole-wizard sketch, printing null, empty and length distinctly; and `GetPersistReference3` byte lengths for one instance of each of the seven new entity kinds. **Activates no sheet, opens no document, changes no display state**, and its own gate log is printed at the end so the run proves it - the distinct interop members, the mutating ones among them, every guard refusal, and one line per claim naming the members it watched for. The document must **already be open**: a `--doc` naming one that is not is refused with exit 1, because `SwSession`'s attach would otherwise open it read-only and a run that opened a document cannot be the proof that it opened none. |
