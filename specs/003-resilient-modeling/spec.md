...
- **Type Table**: The disjoint classification of type names into sketch, solid, cut, hole, fillet, chamfer, shell, draft, pattern, reference, and construction, plus ambiguous types, tolerated loose types, excluded default names, the constrained-status mapping, and the mate entity-kind lists, with the calibrated version. *Amended 2026-09-25 (owner decision 20A):* the tolerated loose types are one list, read by this feature's rules and by feature 004's planner alike; the table carries no second, planner-only list of types that are not content. Until T091 lands (it waits on the owner's question 20A-Q1 with T092), decision 17A's `remodel_not_content` is still that second list.
